...
class SessionAdministrateuristrativeController extends Controller
{
    public function index()
    {
        $sessions = SessionAdministrateuristrative::with('creePar')->latest()->get();
        return view('sessions_Administrateuristratives.index', compact('sessions'));
    }

    public function create()
    {
        return view('sessions_Administrateuristratives.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if (SessionAdministrateuristrative::chevaucheUneSessionExistante($dateDebut, $dateFin)) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Cette période chevauche une session déjà existante. Une seule session peut couvrir une date donnée.');
        }

        $session = SessionAdministrateuristrative::create([
        ...
        return redirect()
            ->route('sessions_Administrateuristratives.index')
    ...
    {
        $session = SessionAdministrateuristrative::with('creePar')->findOrFail($id);
        return view('sessions_Administrateuristratives.show', compact('session'));
    }
    ...
    {
        $session = SessionAdministrateuristrative::findOrFail($id);
